 ...
 * Reads from plain localStorage (consistent with auth provider).
 */
...
export const dataProvider: DataProvider = {
  getList: async ({ resource, pagination, filters }) => {
    ...
    filters?.forEach((filter) => {
      ...
      if (resource === "subjects") {
        ...
        if (["name", "code", "search"].includes(field)) params["search"] = v;
      } else if (resource === "classes") {
        if (["name", "search"].includes(field)) params["search"] = v;
        ...
        if (["name", "email", "search"].includes(field)) params["search"] = v;
        if (field === "status")                      params["status"] = v;
  ...
  create: async ({ resource, variables }) => {
    const res = await fetch(buildUrl(`api/${resource}`), {
      method: "POST", headers: authHeaders(), body: JSON.stringify(variables),
    });
    const body = await handleResponse<any>(res);
  ...
  getOne: async ({ resource, id }) => {
    const res = await fetch(buildUrl(`api/${resource}/${id}`), { headers: authHeaders() });
    const body = await handleResponse<any>(res);
    return { data: body.data ?? body };
  },
  ...
  update: async ({ resource, id, variables }) => {
    const res = await fetch(buildUrl(`api/${resource}/${id}`), {
      method: "PUT", headers: authHeaders(), body: JSON.stringify(variables),
    });
    const body = await handleResponse<any>(res);
    return { data: body.data ?? body };
  },
