...
import { Mail, Calendar, GraduationCap, School, ShieldCheck, BookOpen, Clock } from "lucide-react";
...
export default function UsersShow() {
  ...
  const classesQuery = useList<ClassDetails>({
    ...
    queryOptions: { enabled: !!userData && (userData.role === "teacher") },
  });
  const classesData = classesQuery.result?.data;
  const classesLoading = classesQuery.query?.isLoading;
      ...
      <div className="max-w-2xl mx-auto space-y-4 mt-2">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <Card className="border-0 shadow-sm">
            <CardHeader className="pb-3"><CardTitle className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Contact</CardTitle></CardHeader>
          ...
          <Card className="border-0 shadow-sm">
            <CardHeader className="pb-3"><CardTitle className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Account</CardTitle></CardHeader>
            <CardContent className="space-y-3">
        ...
        {role === "teacher" && (
          <Card className="border-0 shadow-sm">
            <CardHeader className="pb-3">
              <CardTitle className="text-sm font-semibold flex items-center gap-2"><BookOpen className="w-4 h-4 text-primary" />Teaching Classes</CardTitle>
            </CardHeader>
            <CardContent>
              {classesLoading ? <div className="space-y-2">{[...Array(3)].map((_, i) => <Skeleton key={i} className="h-12 w-full" />)}</div>
                : !classesData?.filter((c: any) => c.teacher?.id === userData.id || c.teacherId === userData.id).length
                ? <p className="text-sm text-muted-foreground">No classes assigned.</p>
                : (
                  <div className="space-y-2">
                    {classesData
                      .filter((c: ClassDetails) => c.teacher?.id === userData.id || c.teacherId === userData.id)
                      .map((cls: ClassDetails) => (
