...
export default function Dashboard() {
  ...
  const teachersQuery = useList<User>({ resource: "users", filters: [{ field: "role", operator: "eq", value: "teacher" }], pagination: { pageSize: 5 } });
  const studentsQuery = useList<User>({ resource: "users", filters: [{ field: "role", operator: "eq", value: "student" }], pagination: { pageSize: 1 } });
  ...
  const recentUsers   = recentUsersQuery?.result?.data ?? [];
          ...
          {isAdmin && recentUsers.length > 0 && (
            <Card className="border-0 shadow-sm">
              ...
              <CardContent className="px-4 pb-4">
                <div className="space-y-0.5">{recentUsers.map(u => <UserRow key={u.id} user={u} />)}</div>
              </CardContent>
            </Card>
          )}
        </div>
