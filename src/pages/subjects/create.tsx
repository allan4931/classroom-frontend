...
export default function SubjectsCreate() {
  ...
  const { register, handleSubmit, control, setValue, watch, formState: { errors, isSubmitting } } = useForm<FV>({ resolver: zodResolver(schema) });
  ...
  const onSubmit = (v: FV) => {
    ...
    createMutation.mutate(
      ...
      { onSuccess: () => { setDone(true); setTimeout(() => list("subjects"), 1200); }, onSettled: () => setIsLoading(false) }
    );
  };

  if (done) return <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4"><div className="w-16 h-16 rounded-full bg-green-100 flex items-center justify-center"><CheckCircle2 className="w-8 h-8 text-green-600" /></div><h2 className="text-xl font-bold">Subject Created!</h2></div>;
  ...
  return (
    <div className="container mx-auto max-w-xl px-4 pb-12">
      <div className="flex items-center gap-3 py-6">
        ...
        <div><h1 className="text-2xl font-bold">Create Subject</h1><p className="text-sm text-muted-foreground">Add a subject to a department</p></div>
      </div>
      <form onSubmit={handleSubmit(onSubmit)} noValidate>
        <Card className="border-0 shadow-sm">
          <CardContent className="pt-6 space-y-5">
            <div className="flex items-center gap-2 mb-2"><div className="p-1.5 rounded-md bg-violet-500/10"><BookOpen className="w-4 h-4 text-violet-600" /></div><h3 className="font-semibold">Subject Details</h3></div>
            <div className="grid sm:grid-cols-2 gap-4">
              <F label="Subject Name" required error={errors.name?.message}><Input placeholder="Introduction to Biology" {...register("name")} /></F>
            ...
            <Button type="submit" size="lg" className="w-full" disabled={isSubmitting || isLoading}>
              {(isSubmitting || isLoading) ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />Creating…</> : "Create Subject"}
            </Button>
          </CardContent>
        </Card>
      </form>
